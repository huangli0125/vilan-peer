...
func openPort(name string, baud int, databits byte, parity Parity, stopbits StopBits, readTimeoutMs uint32) (p *Port, err error) {
	...
	if err = setCommTimeouts(h, readTimeoutMs); err != nil {
		return nil, err
	}
...
func setCommState(h syscall.Handle, baud int, databits byte, parity Parity, stopbits StopBits) error {
	...
	r, _, err := syscall.SyscallN(nSetCommState, uintptr(h), uintptr(unsafe.Pointer(&params)))
	if r == 0 {
		return err
	}
	return nil
...
func setCommTimeouts(h syscall.Handle, readTimeoutMs uint32) error {
	...
	*/
	...
	r, _, err := syscall.SyscallN(nSetCommTimeouts, uintptr(h), uintptr(unsafe.Pointer(&timeouts)))
	if r == 0 {
		return err
	}
	return nil
...
func setCommMask(h syscall.Handle) error {
	...
	r, _, err := syscall.SyscallN(nSetCommMask, uintptr(h), EV_RXCHAR)
	if r == 0 {
		return err
	}
	return nil
...
func purgeComm(h syscall.Handle) error {
	...
	r, _, err := syscall.SyscallN(nPurgeComm, uintptr(h), PURGE_TXABORT|PURGE_RXABORT|PURGE_TXCLEAR|PURGE_RXCLEAR)
	if r == 0 {
		return err
	}
	return nil
...
func getOverlappedResult(h syscall.Handle, overlapped *syscall.Overlapped) (int, error) {
	var n int
	r, _, err := syscall.SyscallN(nGetOverlappedResult, uintptr(h), uintptr(unsafe.Pointer(overlapped)), uintptr(unsafe.Pointer(&n)), 1)
	if r == 0 {
		return n, err
	}
